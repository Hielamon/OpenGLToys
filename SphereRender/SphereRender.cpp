...
void TestThread1()
{
	...
	CameraFBO cam(1080, 720, "TestThread1");
...
void TestThread2()
{
	...
	if (!showModel)
	{
		...
		std::shared_ptr<Texture> pTex = std::make_shared<Texture>("./timg.jpg", TextureType::Tex_DIFFUSE);
		std::shared_ptr<Material> pMatrial = std::make_shared<Material>();
		pMatrial->addTexture(pTex);
		std::shared_ptr<Mesh> pMesh = std::make_shared<Mesh>(pVA, pMatrial);
		pScene->addMesh(pMesh);
	...
	CameraFBO cam(1080, 720, "TestThread2");
	//cam.setViewMatrix(glm::vec3(0.0f, 0.0f, 5.0f), glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f));
	glm::mat4 model;
	model = glm::translate(model, glm::vec3(0.0f, 0.0f, 0.9f)); // translate it down so it's at the center of the scene
																//model = glm::scale(model, glm::vec3(0.2f, 0.2f, 0.2f));
...
void TestThread3(std::shared_ptr<Scene> pScene)
{
	

	CameraFBO cam(480, 320, "TestThread3");
	HL_INTERVAL_START
		cam.setScene(pScene);
	HL_INTERVAL_ENDSTR("cam.setScene(pScene)");
...
void TestThread4(std::shared_ptr<Scene> pScene)
{
	CameraFBO cam(1080, 720, "TestThread4");
	HL_INTERVAL_START
		cam.setScene(pScene);
	HL_INTERVAL_ENDSTR("cam.setScene(pScene)");
...
int main(int argc, char *argv[])
{
	...
	//1080, 720
	//1920, 1080
	CameraFBO cam(1920, 1080, "TestThread1");
	//cam.setViewMatrix(glm::vec3(0.0f, 0.0f, 5.0f), glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f));
	glm::mat4 model;
	model = glm::translate(model, glm::vec3(0.0f, 0.0f, 0.9f)); // translate it down so it's at the center of the scene
																//model = glm::scale(model, glm::vec3(0.2f, 0.2f, 0.2f));
